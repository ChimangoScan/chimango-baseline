#!/usr/bin/env python3
"""Extra analyses for the random-sample control short paper, plus one compact
3-panel figure (fig_extra.pdf). All numbers on the N=2876 random images.
...
OUT = os.environ.get("BL_FIGS", os.path.join(_HERE, "figures"))
...
# (1) scanner divergence
overall_sets = {s: set() for s in VULN_SCANNERS}          # distinct CVEs overall, per scanner
img_share = Counter()        # over (image, cve): how many of the 3 scanners reported it -> count
img_share_imglevel = Counter()  # average per-image: bucket counts of per-image distinct CVEs
n_img_with_vuln = 0
percell_by1 = []; percell_by2 = []; percell_by3 = []  # per-image fractions (images with >=1 cve)

# (2) OS distribution
img_os = []                  # one dominant family per image (or None)

# (3) misconfig
dockle_id_imgcount = Counter()   # images that have each dockle id
dockle_title = {}
n_img_misconf = 0
n_img_dockle_ran = 0
# count excluding the near-universal content-trust check
n_img_misconf_strict = 0

# scanner participation (how many of the 3 vuln scanners ran ok per image) for context
...
for (rj,) in con.execute("SELECT report_json FROM reports"):
    ...
    # which vuln scanners ran ok on this image
    ...
    # per-image distinct CVEs per scanner
    per_img = {s: set() for s in VULN_SCANNERS}
    distro_pkg_counter = Counter()   # count OS packages by distro family
    dockle_ids_here = set()
    ...
    for f in findings:
        ...
        if cat == "pkg-vuln" and sc in VULN_SCANNERS:
            cves = f.get("cves") or []
            # the id itself is frequently the CVE; fold it in if it matches
            fid = f.get("id") or ""
    ...
    # (1) tally per (image, cve) how many of trio reported it -- restrict to the
    # scanners that actually ran ok so a missing scanner does not inflate "by-1".
    ...
    # (2) dominant OS family for this image
    ...
    if dockle_ids_here:
        n_img_misconf += 1
        strict = dockle_ids_here - {"CIS-DI-0005"}  # drop near-universal content-trust check
        if strict:
...
# ============================================================ (1) DIVERGENCE
# overall (pooled across all images) distinct CVE union
...
vol = {s: len(overall_sets[s]) for s in VULN_SCANNERS}
...
pi1, pi2, pi3 = np.mean(percell_by1), np.mean(percell_by2), np.mean(percell_by3)
